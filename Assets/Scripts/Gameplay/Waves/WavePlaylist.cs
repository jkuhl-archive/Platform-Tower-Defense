...
namespace Gameplay.Waves
{
    ...
    {        
        ...
        private static string jsonDirectory = Path.Combine(Application.dataPath, "Json", "WavePlaylists");
        ...
        public bool CanSpawnCreep()
        {
            ...
            {
                return waveDataList[waveDataIndex].HasAvailableSpawn();
            }
        ...
        public WaveSpawnObject GetNextSpawn()
        {
            ...
            {
                WaveSpawnObject nextSpawn = waveDataList[waveDataIndex].GetNextSpawn();
        ...
        {
            if (waveDataList[waveDataList.Count - 1].RewardGiven())
            {
            ...
            if (waveActive)
            {
                return waveDataList[waveDataIndex].waveMessage;
            }
        ...
        {
            string filePath = Path.Combine(jsonDirectory, jsonFileName);
            
            Debug.Log($"Loading WavePlaylist data from JSON file: '{filePath}'");

            if (File.Exists(filePath))
            {
                return JsonUtility.FromJson<WavePlaylist>(File.ReadAllText(filePath));
            }
